...
def speaker_gender_lines(pattern='kvinna', lower=1900, upper=1940):
    with sqlite3.connect(tmp_db) as conn:
        ...
        for gender in genders:
            ark = cur.execute(
                f'select party, count(*) as c from utterance as u join utterance_fts as uf on u.rowid == uf.rowid where gender == "{gender}" and content match "{pattern}" and year between {lower} and {upper} group by who order by c desc limit 10').fetchall()
            if ark == []:
                continue
